...
void query(int a,int b){
    ...
    for(idx=19;idx>=0;idx--){
        if(ancestor[a][idx] != ancestor[b][idx]){
    ...
    maxi = max(maxi,max_distance[a][0]);
    maxi = max(maxi,max_distance[b][0]);

    mini = min(mini,min_distance[a][0]);
    mini = min(mini,min_distance[b][0]);
